...
export function ProductCard({ product, locale }: { product: Product; locale: Locale }) {
  ...
  return (
    <article className="group flex flex-col rounded-sm border border-white/10 bg-stone-900 overflow-hidden hover:border-rakura-gold/50 hover:-translate-y-0.5 transition-all duration-300">
      {/* Product image */}
      <div className="aspect-[3/4] relative bg-white overflow-hidden">
        {hasImage ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-stone-400 font-display font-bold text-lg tracking-widest opacity-30">RAKURA</span>
          </div>
        )}
      </div>
      ...
      <div className="p-4 flex flex-col flex-1 border-t border-white/10">
        <h3 className="font-semibold text-white text-sm leading-snug group-hover:text-rakura-gold transition-colors">
          {name}
        </h3>
        {product.bagCount != null && (
          <p className="text-xs text-rakura-gold/70 mt-1 tracking-wide">{product.bagCount} {t.bags}</p>
        )}
        {product.weight != null && (
          <p className="text-xs text-rakura-gold/70 mt-1 tracking-wide">{product.weight}</p>
        )}
        {showDescription && (
          <p className="text-xs text-stone-400 mt-2 line-clamp-2 leading-relaxed">{description}</p>
        )}
        <Link
          href={inquireUrl}
